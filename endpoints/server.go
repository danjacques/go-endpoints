...
type Server struct {
	...
	// context, and the resulting Context is forwarded to the handler.
	//
	// If an error is returned, it is handled in the same manner as if the handler
	// had returned an error.
	ContextFunc func(context.Context, *ServiceInfo, *MethodInfo) (context.Context, error)
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if s.ContextFunc != nil {
		var err error
		c, err = s.ContextFunc(c, serviceSpec.info, methodSpec.info)
		if err != nil {
			writeError(w, err)
			return
		}
